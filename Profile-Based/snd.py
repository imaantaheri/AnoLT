...
def process_file(file_path):
    ...
    # Calculate mean and standard deviation for each (Weekday, Time) group
    stats_table = train.groupby(['Weekday', 'Time'])['Volume'].agg(
        Mean='mean',
        Std='std'
    ).reset_index()

    # Merge stats into test set
    test = test.merge(stats_table, on=['Weekday', 'Time'], how='left')
    test.dropna(subset=['Mean', 'Std'], inplace=True)

    # Create AND and OR label combinations
    test = test.assign(
    ...
    for idx, label in enumerate(label_sets):
        ...
        best_result = None
        for c in c_values:
            upper = test['Mean'] + c * test['Std']
            lower = test['Mean'] - c * test['Std']
