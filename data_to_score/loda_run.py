...
n_bins = [5, 7, 10, 15, 20, 25, 30]
n_random_cuts = [20, 50, 70, 100, 120, 150, 200]
window_size = [3, 4, 5, 7, 10]
#window_size = [4]

hyp_list = [window_size, n_bins, n_random_cuts]
...
for location in range(len(dir_list)):
    ...
    for hyp in range(len(combinations)):
        ...
        label = roll_binary_window(data, label_set_1, combinations[hyp][0])
        contamination = df_percentage_of_ones(label, label_set_1)
        
        transformer = LODASKI(n_bins = combinations[hyp][1],
                              n_random_cuts = combinations[hyp][2],
    ...
    with open('loda_results/' + dir_list[location][0:-4] + '.txt', mode='w') as file:
