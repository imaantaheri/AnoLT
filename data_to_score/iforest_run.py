...
hyp_list = [bootstrap, behaviour, window_size]
...
for location in range(len(dir_list)):
    ...
    for hyp in range(len(combinations)):
        ...
        X_train = roll_window(X_train_main, combinations[hyp][2])
        X_test = roll_window(X_test_main, combinations[hyp][2])
        
        label = roll_binary_window(data, label_set_1, combinations[hyp][2])
        contamination = df_percentage_of_ones(label, label_set_1)
        
        transformer = IsolationForestSKI(bootstrap = combinations[hyp][0], 
                                        behaviour = combinations[hyp][1],
    ...
    with open('iforest_results/' + dir_list[location][0:-4] + '.txt', mode='w') as file:
