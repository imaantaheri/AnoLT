...
hyp_list = [window_size, step_size, n_neighbors, method]
...
for location in range(len(dir_list)):
    ...
    for hyp in range(len(combinations)):
        ...
        transformer = KDiscordODetectorSKI(window_size = combinations[hyp][0],
                                        step_size = combinations[hyp][1], 
                                        n_neighbors = combinations[hyp][2],
                                        method = combinations[hyp][3],
    ...
    with open('discord_results/' + dir_list[location][0:-4] + '.txt', mode='w') as file:
