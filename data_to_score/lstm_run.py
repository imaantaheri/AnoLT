...
#window_size = [3]
n_hidden_layer = [3, 5, 10]
#n_hidden_layer = [3]
min_attack_time = [1, 2, 3, 4]
#min_attack_time = [2]

hyp_list = [hidden_dim, epochs, window_size, n_hidden_layer, min_attack_time]
...
def process_dataset(location):
    ...
    for hyp in range(len(combinations)):
        ...
        transformer = LSTMODetectorSKI( hidden_dim = combinations[hyp][0], 
                                        epochs = combinations[hyp][1], 
                                        window_size = combinations[hyp][2], 
                                        n_hidden_layer = combinations[hyp][3],
                                        min_attack_time = combinations[hyp][4], 
                                        contamination = contamination,
                                        train_contamination = contamination, 
                                        )
    ...
    with open('lstm_results/' + dir_list[location][0:-4] + '.txt', mode='w') as file:
