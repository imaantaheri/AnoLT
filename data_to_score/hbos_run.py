...
for location in range(len(dir_list)):
    ...
    for hyp in range(len(combinations)):
        ...
        X_train = roll_window(X_train_main, combinations[hyp][3])
        X_test = roll_window(X_test_main, combinations[hyp][3])
        
        label = roll_binary_window(data, label_set_1, combinations[hyp][3])
        contamination = df_percentage_of_ones(label, label_set_1)
        
        transformer = HBOSSKI(n_bins = combinations[hyp][0],
                              alpha = combinations[hyp][1],
    ...
    with open('hbos_results/' + dir_list[location][0:-4] + '.txt', mode='w') as file:
